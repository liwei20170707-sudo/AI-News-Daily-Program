...
def push_pushplus_email(config, title, html_content):
    """PushPlus邮箱推送（使用POST方式）"""
    token = config['push']['pushplus_token']
    if not token:
        print("⚠️ PushPlus Token未配置")
        return False
    ...
    for email in emails:
        ...
        try:
            ...
            if '"code":200' in result:
                print(f"✅ PushPlus邮箱推送成功: {email}")
            else:
                print(f"⚠️ PushPlus推送失败 ({email}): {result}")
        except Exception as e:
            print(f"❌ PushPlus推送失败 ({email}): {e}")
...
def main():
    ...
    # 2. PushPlus邮箱推送
    push_pushplus_email(config, title, html_report)
